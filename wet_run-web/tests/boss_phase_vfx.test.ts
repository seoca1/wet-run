/** Unit tests for boss 4-phase VFX (Tier 5.5+). */
...
describe("boss phase 4-phase VFX (Tier 5.5+)", () => {
  describe("renderCombatVfx boss_phase_1 (calm sentinel)", () => {
    it("uses header containing PHASE 1", () => {
      const inst = triggerCombatVfx("boss_phase_1", "", 5);
  ...
  describe("renderCombatVfx boss_phase_2 (alert, yellow-amber)", () => {
    it("contains PHASE 2 + alert message", () => {
      const inst = triggerCombatVfx("boss_phase_2", "", 5);
      const grid = renderCombatVfx(inst, 80, 50);
      const header = findLine(grid, 80, 1);
  ...
  describe("renderCombatVfx boss_phase_3 (enraged, red-bright)", () => {
    it("contains PHASE 3 + enrage message", () => {
      const inst = triggerCombatVfx("boss_phase_3", "", 5);
      const grid = renderCombatVfx(inst, 80, 50);
      const header = findLine(grid, 80, 1);
    ...
    it("renders red flicker row on tick 0 (Phase 3 special effect)", () => {
      const inst = triggerCombatVfx("boss_phase_3", "", 5);
      const grid = renderCombatVfx(inst, 80, 50);
      let hasEnrage = false;
      for (let x = 0; x < 80; x++) {
  ...
  describe("renderCombatVfx boss_phase_4 (desperate, magenta-neon)", () => {
    it("contains PHASE 4 + desperation message", () => {
      const inst = triggerCombatVfx("boss_phase_4", "", 5);
      const grid = renderCombatVfx(inst, 80, 50);
      const header = findLine(grid, 80, 1);
    ...
    it("renders magenta pulse on tick 0 (Phase 4 special effect)", () => {
      const inst = triggerCombatVfx("boss_phase_4", "", 5);
      const grid = renderCombatVfx(inst, 80, 50);
      let hasPulse = false;
      for (let x = 0; x < 80; x++) {
  ...
  describe("phase color differentiation", () => {
    it("phase 1 and 4 use different colors (cyan vs magenta)", () => {
      const inst1 = triggerCombatVfx("boss_phase_1", "", 5);
      const inst4 = triggerCombatVfx("boss_phase_4", "", 5);
      const grid1 = renderCombatVfx(inst1, 80, 50);
      const grid4 = renderCombatVfx(inst4, 80, 50);
      // Header at center; sample 3 cells.
      const cx = 40;
