...
- **2026-09-24 (web/ 구조 정합 — CI/배포 복구 + 데드코드 정리 + 문서 동기화)**: 프로토타입 통합 직후 잔재 정리 일괄 처리. 다음 항목들은 모두 사전에 Python 프로토타입 시절의 historical bullets 이다 — 본 세션은 web/ 구조 정합이 본질.
  - **CI / 배포 정정**: GitHub Actions CI 를 web 전용으로 축소 (Python job 제거, 더 이상 존재하지 않는 Python 패키지용 release 워크플로 제거). Pages 배포는 web-only 로 정정. `mkdocs.yml` 의 `docs_dir` 를 현행 트리에 맞춰 정정.
  - **데드코드 제거**: `run_validation.py` 와 `web/src/core/equip_slots.ts` 등 dead file/code 정리. `web/src/core/data_loaders.ts` 와 `web/src/core/starter_deck.ts` 의 provenance 주석 보강.
  - **문서 동기화**: 테스트 수를 **107 파일 / 2654 tests / 0 failures** 로 통일 (이전 카운트 드리프트 해소). `prototype/` 가 2026-09-15 통합으로 삭제되었음을 AGENTS/README 트리 다이어그램에 명시. 본 AGENTS.md §2 / §4 / §5 의 stale 경로 (`raw/`, `wiki/`, `design/`, `testcases/`, `decisions/`, `prototype/`, `dashboard/`) 를 현행 트리 (`web/`, `docs/`, `data/`, `scripts/`, 메타 파일) 로 교체.
  - **검증**: `npm run typecheck` 0 errors / `npm run lint` 0 errors (4 warnings) / `npm test` 2654 pass / `npm run build` OK / `mkdocs build --strict` OK.
  - **밸런스 조사**: 비단조 승률 (grade 4 → grade 5 승률 역전) 의 근본 원인은 `dungeon.ts` 의 ICE 하드코딩 + `parseIce` 의 `hp=100` 디폴트 + `ice_types.json` 의 `hp_per_grade` 필드 미사용 (dead data). Draft ADR-0211 + 진단 보고서 `docs/diagnostics/balance-grade-curve-root-cause-2026-09-24.md` 기록.
